...
// Include these for coverage stuff
#include <fstream>
#include <iostream>
...
static std::vector<bb_entry> bb_list;
...
void Jit64::Init()
{
  // Clear the coverage list
  bb_list.clear();
  NOTICE_LOG(CONSOLE, "Coverage data cleared!");
...
u8* Jit64::DoJit(u32 em_address, JitBlock* b, u32 nextPC)
{
  ...
  if (SConfig::GetInstance().bEnableCodeCoverage)
  {
    // Add instruction block execution to coverage list
    bb_list.push_back(bb_entry());
    size_t size = bb_list.size();
    bb_list[size - 1].addr = js.blockStart;
    bb_list[size - 1].count = js.downcountAmount * 4;
  }
...
void Jit64::SaveCoverageData(std::string file_path)
{
  // Open up the DOL
  std::string filename = file_path.substr(file_path.rfind("/") + 1);

  FILE* dol_file = fopen(file_path.c_str(), "rb");

  // Fetch the DOL's entry point
  u32 ep_addr, tmp;
  fseek(dol_file, 0x48, SEEK_SET);
  fread(&tmp, sizeof(u32), 1, dol_file);
  fclose(dol_file);

  // Convert LE address to BE address
  ep_addr = ((tmp & 0x000000FF) << 24) | ((tmp & 0x0000FF00) << 8) | ((tmp & 0x00FF0000) >> 8) |
            ((tmp & 0xFF000000) >> 24);

  std::ofstream debug_file;
  std::string new_filename;
  std::string write_status = "Writing coverage data to ";

  // Infinite loop to get a unique filename
  int n = 0;
  while (true)
  {
    // Filename format is example.dol.0.log
    new_filename = filename + "." + std::to_string(n) + ".log";

    // Filename exists, go to next one
    if (FILE* test_file = fopen(new_filename.c_str(), "r"))
    {
      fclose(test_file);
      n++;
    }
  ...
  write_status += new_filename + "...";
  NOTICE_LOG(POWERPC, "%s", write_status.c_str());

  debug_file.open(new_filename);
  debug_file.imbue(std::locale("C"));

  // Write the header
  debug_file << "DRCOV VERSION: 2\n";
  debug_file << "DRCOV FLAVOR: drcov\n";
  debug_file << "Module Table: version 4, count 1\n";
  debug_file
      << "Columns: id, containing_id, start, end, entry, offset, checksum, timestamp, path\n";

  // Write out the module info
  debug_file << std::setfill(' ') << std::dec << std::setw(3) << 0 << ", ";
  debug_file << std::setfill(' ') << std::dec << std::setw(3) << 0 << ", ";
  debug_file << std::noshowbase << std::internal << std::setfill('0');
  debug_file << "0x" << std::hex << std::setw(16) << 0 << ", ";
  debug_file << "0x" << std::hex << std::setw(16) << 0 << ", ";
  debug_file << "0x" << std::hex << std::setw(16) << 0 << ", ";
  debug_file << std::hex << std::setw(16) << 0 << ", ";
  debug_file << "0x" << std::hex << std::setw(8) << 0 << ", ";
  debug_file << "0x" << std::hex << std::setw(8) << 0 << ", ";
  debug_file << file_path << '\n';

  // Write out the BB list info
  debug_file << "BB Table: " << std::dec << bb_list.size() << " bbs\n";

  // Module coverage header
  debug_file << "module id, start, size:\n";

  // Write actual coverage info
  for (u32 i = 0; i < bb_list.size(); i++)
  {
    u32 offset = bb_list[i].addr - ep_addr;
    debug_file << "module[  0]: "
               << "0x" << std::setfill('0') << std::hex << std::setw(16) << offset << ", "
               << std::setfill(' ') << std::dec << std::setw(3) << bb_list[i].count << '\n';
  }

  debug_file.close();

  NOTICE_LOG(POWERPC, "Finished writing coverage data!");
}
